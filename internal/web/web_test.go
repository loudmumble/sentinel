...
func TestHealthEndpoint(t *testing.T) {
	...
	if data["service"] != "sentinel-ebpf" {
		t.Errorf("expected service 'sentinel-ebpf', got %v", data["service"])
	}
	if data["version"] != "0.3.0b1" {
		t.Errorf("expected version '0.3.0b1', got %v", data["version"])
	}
}

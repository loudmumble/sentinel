...
import (
	...
	ebpfcore "github.com/loudmumble/syscalld/core"
	ebpfsensors "github.com/loudmumble/syscalld/sensors"

	"github.com/loudmumble/sentinel/internal/events"
)

// EBPFSensorAdapter integrates ebpf-sensors into Sentinel as a probe.
...
// Usage:
//
//	adapter := probes.NewEBPFSensorAdapter(nil)
//	adapter.Start()
//	evts := adapter.Poll()   // call periodically
//	adapter.Stop()
type EBPFSensorAdapter struct {
	mgr    *ebpfcore.SensorManager
	buf    chan events.EventInterface
	mu     sync.Mutex
	stopCh chan struct{}
}

// NewEBPFSensorAdapter creates an adapter with all 7 sensors registered.
// Pass a *ebpfcore.SensorFilter to restrict monitoring scope, or nil for defaults.
func NewEBPFSensorAdapter(filters *ebpfcore.SensorFilter) *EBPFSensorAdapter {
	mgr := ebpfcore.NewSensorManager(filters)
	mgr.Add(ebpfsensors.NewSyscallSensor())
	mgr.Add(ebpfsensors.NewProcessSensor())
	mgr.Add(ebpfsensors.NewFilesystemSensor())
	mgr.Add(ebpfsensors.NewNetworkSensor())
	mgr.Add(ebpfsensors.NewMemorySensor())
	mgr.Add(ebpfsensors.NewModuleSensor())
	mgr.Add(ebpfsensors.NewDnsSensor())
	...
	mgr.OnAny(func(e ebpfcore.Event) {
		se := convertEvent(e)
		if se == nil {
			return
		}
...
func (a *EBPFSensorAdapter) Healths() []ebpfcore.SensorHealth {
	return a.mgr.Healths()
}

// ---------------------------------------------------------------------------
// Event conversion — ebpf-sensors core.Event → sentinel events.EventInterface
// ---------------------------------------------------------------------------

// convertEvent converts an ebpf-sensors event to a Sentinel EventInterface.
// Returns nil for unhandled or canary events (canary is infrastructure-only).
func convertEvent(e ebpfcore.Event) events.EventInterface {
...
func convertSyscall(e ebpfcore.Event) events.EventInterface {
	m := e.ToSentinelEvent()
...
func convertProcess(e ebpfcore.Event) events.EventInterface {
	m := e.ToSentinelEvent()
...
func convertFile(e ebpfcore.Event) events.EventInterface {
	m := e.ToSentinelEvent()
...
func convertNetwork(e ebpfcore.Event) events.EventInterface {
	m := e.ToSentinelEvent()
